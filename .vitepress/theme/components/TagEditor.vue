...
import { generateNewTagsFromGPT, saveTags, tagsCanBeGenerated } from '../../plugins/vitepress-plugin-docsmd-easytag/src/api/client/tags'
import BasicButton from './BasicButton.vue'

interface Tag { id: string; content: string }
...
onMounted(async () => {
  ...
  if (
    ...
    tags.value = pageData.frontmatter.value.tags.map((tag: string) => {
      return { id: String(uuidv4()), content: tag }
    })
    suggestToGenerate.value = false
  }
  ...
  try {
    ...
    if (canBeGeneratedRes.canBeGenerated)
      canBeGenerated.value = true
  }
  catch (err) {
    console.error(err)
  }
...
<template>
  <div v-if="!initLoading && ((suggestToGenerate && canBeGenerated) || (tags && tags.length > 0))">
    <div flex="~ row" items-center>
      ...
      <BasicButton
        v-if="tags && tags.length > 0 && (!editingTags || (!editingTags && generated))" title="编辑" flex="~ row"
      ...
      <BasicButton
        v-if="editingTags" title="取消" flex="~ row" items-center justify-center text="dark:white"
      ...
      <BasicButton
        v-if="editingTags" title="保存" flex="~ row" ml-2 items-center justify-center text="dark:white"
    ...
    <section>
      <div
        v-if="!initLoading && suggestToGenerate && canBeGenerated" dark="bg-zinc-900"
        my-4 flex rounded-lg bg-zinc-50 p-4
      >
        <div flex="~ 1 grow col">
          ...
          <span v-if="!generated && generatedError" flex="~ 1" mt-2 items-center>
        ...
        <div v-if="!generated">
          <BasicButton
            title="生成" text="dark:white" bg="yellow-500 dark:yellow-600"
        ...
        <div v-if="generated && editingTags" flex flex-row items-center justify-center>
          <BasicButton
            mr-2 title="重新生成" text="dark:white" bg="blue-500 dark:blue-600"
      ...
      <div v-if="tags && tags.length > 0" my-4 rounded-lg bg-zinc-50 p-4 dark="bg-zinc-900">
        <transition-group>
          <Draggable
            key="draggable" v-model="tags" animation="200" group="tags" ghost-class="tags-ghost" item-key="id"
        ...
        <TagItem v-if="editingTags">
          <template #default>
            <form inline flex flex-row items-center justify-center @submit.prevent="addTag">
