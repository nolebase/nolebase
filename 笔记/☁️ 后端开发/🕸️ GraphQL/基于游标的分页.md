...
#KeysetPagination #SeekMethod #CursorBasedPagination #密钥集分页 #键集分页 #搜寻方式 #基于游标分页 #graphql #postgres #postgresql #sql #慢查询 #瀑布流 #分页
...
这个公式本质上是 PostgreSQL 封装的语法糖，如果我们要使用拓展写法的话，应该对照下面的转换案例进行转换[^7]：
...
```sql
WHERE x >= a AND (x = a AND y > b OR x > a)
```
